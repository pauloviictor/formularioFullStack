...
function salvaDados(e) {
    ...
    if (validaCpf() && validaNome() && validaEmail() && quantContatos.value >= 1) {
        ...
        if (idElemento.value == 0) {
            ...
            fetch("/formulario/rest/pessoa/novoCadastro", {
                method: "POST",
            ...
            fetch("/formulario/editaCadastro", {
                method: "POST",
                body: JSON.stringify(cadastro)
            })
                .then(() => exibeCadastros())
...
function buscaCadastro(id) {
    ...
    fetch("/formulario/cadastro?id=" + id + "")
        .then(response => {
            response.json().then(json => {
...
function removerCadastro(id) {
    fetch("/formulario/removeCadastro?id=" + id + "")
        .then(() => exibeCadastros())
        .catch(err => console.log(err))
}
